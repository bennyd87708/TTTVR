--defines the font and colors for the role ui
...
hook.Add("TTTBeginRound","Benny:TTTVR", function()
	local ply = LocalPlayer()
	if(IsValid(ply) && CLIENT && istable(vrmod)) then
		if(vrmod.IsPlayerInVR(ply)) then
		
			--find player's role and define corresponding ui background color
			...
			--paint the rounded box and shadowed text for what role the player has in the panel
			...
			vrmod.MenuCreate("Benny:TTTVR:roleui", 600, 200, roleui, 2, Vector(10,6,13), Angle(0,-90,50), 0.03, false, function()
			...
			timer.Simple(5, function()
				if(vrmod.MenuExists("Benny:TTTVR:roleui")) then
					vrmod.MenuClose("Benny:TTTVR:roleui")
				end
			end)
