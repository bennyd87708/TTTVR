...
-- on weapon switch, adjust the global muzzle offset to the right numbers for the magneto stick
...
-- switch to c model because the v model has hands
		...
		if pickup then
			if (ply:EyePos() - trace.HitPos):Length() < self:GetRange(ent) then
				...
				if self:AllowPickup(ent) then
					...
					local delay = (ent:GetClass() == "prop_ragdoll") and 2 or 1.5
