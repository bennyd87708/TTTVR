...
-- stolen from entities\ttt_c4\cl_init and modified to support handling regular and VR users
...
-- VR version of ShowC4Config
function ShowTTTVRC4Config(bomb)
	...
	dgrab.DoClick = function()
							 ...
							 vrmod.MenuClose("Benny:TTTVR:bombui")
						 end
	...
	dcancel.DoClick = function() vrmod.MenuClose("Benny:TTTVR:bombui") end

	-- use player's right hand position to calculate where to put the bomb UI
	local tmp = Angle(0,vrmod.GetHMDAng(ply).yaw-90, 45)
	local pos, ang = WorldToLocal(vrmod.GetRightHandPos(ply) + tmp:Forward()*-9 + tmp:Right()*-11 + tmp:Up()*-7, tmp, vrmod.GetOriginPos(), vrmod.GetOriginAng())
	
	-- create the menu
	vrmod.MenuCreate("Benny:TTTVR:bombui", w, h, dframe, 4, pos, ang, 0.03, true, function()
...
function ShowC4Config(bomb)
	...
	-- ShowC4Config is always run when a bomb is opened so check if the player is in VR to use that function instead
	if vrmod.IsPlayerInVR(LocalPlayer()) then
		ShowTTTVRC4Config(bomb)
		return
	end
...
-- VR version of ShowC4Disarm
function ShowTTTVRC4Disarm(bomb)
	local dframe = vgui.Create("DFrame")
	local w, h = 420, 340
	dframe:SetSize(w, h)
	dframe:SetPos(0,0)--
	...
	dgrab.DoClick = function()
							 ...
							 RunConsoleCommand("ttt_c4_pickup", bomb:EntIndex())
							 vrmod.MenuClose("Benny:TTTVR:bombdefuseui")
						 end
	...
	-- use player's right hand position to calculate where to put the bomb defuse UI
	local tmp = Angle(0,vrmod.GetHMDAng(ply).yaw-90, 45)
	local pos, ang = WorldToLocal(vrmod.GetRightHandPos(ply) + tmp:Forward()*-9 + tmp:Right()*-11 + tmp:Up()*-7, tmp, vrmod.GetOriginPos(), vrmod.GetOriginAng())

	-- create the menu using VRMod API
	vrmod.MenuCreate("Benny:TTTVR:bombdefuseui", w, h, dframe, 4, pos, ang, 0.03, true, function()
		if IsValid(dframe) then
			dframe:SetVisible(false)
			dframe:Remove()
			dframe = nil
...
function ShowC4Disarm(bomb)
	...
	-- ShowC4Disarm is always run when a bomb is opened so check if the player is in VR to use that function instead
	if vrmod.IsPlayerInVR(LocalPlayer()) then
		ShowTTTVRC4Disarm(bomb)
		return
	end
